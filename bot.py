...
async def check_data(
    ...
    if update.effective_chat.type == "private" and user_id != OWNER_ID:
        await update.message.reply_text(
            "⛔ Private chat တွင် Owner သာ အသုံးပြုနိုင်ပါသည်။"
        )
        return

    if gsheet_data is None:
        await update.message.reply_text(
            "⚠️ Google Sheet data မရနိုင်သေးပါ။ ခဏစောင့်ပေးပါ။"
        )
        return
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id not in user_states:
        await query.edit_message_text(
            "⏰ Session ကုန်သွားပါပြီ။ /check_data ကို ပြန်နှိပ်ပါ။"
        )
        return
...
def _build_month(state):
    ...
    if sheet == "Testing":
        keyboard.append([
            InlineKeyboardButton(
                "📊 Yearly Total", callback_data=YEARLY_TOTAL_SELECT
            )
        ])
...
def _build_profile_data(state):
    ...
    text += f"🏷 Provider Type: <b>{data.get('Provider Type', 'N/A')}</b>\n"
    text += f"🔢 Provider Code: <b>{data.get('Provider Code', 'N/A')}</b>\n"
